...
img_dir = os.listdir(r'C:\Users\deshc\Desktop\Chinar\Research Track\Damage')
for path in img_dir:
    try:
        img = cv2.imread(r"C:\Users\deshc\Desktop\Chinar\Research Track\Damage" + path)
        ...
        for i in range(len(M)):
            if(not isWheel):
                ...
                for j in range(len(M[i])):
                    if M[i][j] == 255 and (arr[0][0] < j < arr[0][1] or arr[1][0] < i < arr[1][1]):
                        img[i][j] = 0
        
        print("Processed Image " + str(count))
        cv2.imwrite(r"C:\Users\deshc\Desktop\Chinar\Research Track\Predictions" + 'count', img)
        count = count + 1
    except:
        print(path)
